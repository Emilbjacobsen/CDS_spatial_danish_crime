...
voldsforbrydelser <- read_csv2("data/anmeldte_forbrydelser_ialt_år_numbers.csv")
...
# assign numbers as numbers
befolkningstal[,2:21] <- sapply(befolkningstal[ ,2:21], as.numeric)
# getting sum of rows
befolkningstal$crime_sum <- rowSums(befolkningstal[,2:21])
...
df <- mun %>% 
  select(NAME_2) %>% 
  merge(crime_rate, by.x = "NAME_2",by.y ="Municipality")
